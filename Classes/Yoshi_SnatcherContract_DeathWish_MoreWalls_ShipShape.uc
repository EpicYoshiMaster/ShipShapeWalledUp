...
* Shifting Tides: Adds randomly placed walls which prevent the player from traversing certain paths. All rooms are possible to enter in some fashion, though it may be inconvenient!
*/
class Yoshi_SnatcherContract_DeathWish_MoreWalls_ShipShape extends Hat_SnatcherContract_DeathWish
dependsOn(Yoshi_InterpActor_DoomBlockTaskmaster);

const ShipShapeTarget = 18;
const NoMissTarget = 10;

const MaxBlockages = 7;
...
defaultproperties
{
	...
	Objectives(0) = (Title = "Objective", MaxTriggerCount = ShipShapeTarget)
	Objectives(1) = (Title = "NoMiss", MaxTriggerCount = NoMissTarget)
	Objectives(2) = (Title = "FewHits")
    Conditions = ("Condition")
	TipLocalizedMessage = "Tip0"

	AllowedMaps = ("ship_main")
	AllowedActs = (2)
	ParentDeathWishes.Add(class'Hat_SnatcherContract_DeathWish_RiftCollapse_Cruise');
...
function OnMiniMissionBegin(Object MiniMission)
{
    ...
    Hat_MiniMissionTaskMaster(MiniMission).EasyTime = 140; //From 120
    Hat_MiniMissionTaskMaster(MiniMission).HardTime = 210; //From 180
    Hat_MiniMissionTaskMaster(MiniMission).EasySpeed = 8; //From 10
    Hat_MiniMissionTaskMaster(MiniMission).HardSpeed = 30; //From 40
    Hat_MiniMissionTaskMaster(MiniMission).SucceedPressure = 0.16; //From 0.15
    Hat_MiniMissionTaskMaster(MiniMission).FailPressure = 0.06; //Fromt 0.07
...
function OnPostPawnCombatTakeDamage(Pawn PawnCombat, int Damage, Controller InstigatedBy, Vector HitLocation, Vector Momentum, class<Object> DamageType, optional TraceHitInfo HitInfo, optional Actor DamageCauser)
{
	if (IsExcluded()) return;
	if (!PawnCombat.IsA('Hat_Player')) return;

	if (Hat_Player(PawnCombat).Health <= 1)
		SetObjectiveFailed(2, true);
}
